	...
	, mCheckMultilineComments(true)
{
    ...
    mPalette[(int)TokenType::Comment] = 0xff59A64A;
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	if (ImGui::IsWindowHovered())
	{
        ...
        ImVec2 origin = ImGui::GetCursorScreenPos();
        ImVec2 local(mousePos.x - origin.x, mousePos.y - origin.y);
		...
		while (lineNo <= lineMax)
		{
			...
			if (errorIt != mErrorMarkers.end())
			{
				...
				if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
				{
					...
					ImGui::Text("Error at line %d:", errorIt->first - 1);
					ImGui::PopStyleColor();
					ImGui::Separator();
...
{
	mLines.clear();
    if (aText.empty()) mLines.push_back(Line());
	for (auto chr : aText)
	{
		if (mLines.empty())
			mLines.push_back(Line());
...
void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
	...
	for (int i = aFromLine; i < endLine; ++i)
	{
		...
		//{
			...
			{
				if (std::regex_search<std::string::const_iterator>(buffer.cbegin(), buffer.cend(), results, p.first))
				{
